    ...
    <el-dialog :title="title" :visible.sync="dialogTableVisible" class="dialog-barcode" :before-close="() => confirm(false)" width="580px">
        <div id="ProcurementDocuments">
            <Header>
                <h2>{{ title }}</h2>
                <div>
                    <div v-if="title === '采购入库单' ">
                        <p>{{detail.purchaser}}</p>
                        <p>{{ detail.whouseName }}</p>
                    </div>
                     <div :class="{'new-width': title === '质检异常单' }" v-else-if="title === '质检异常单'" class="qaer">
                        <p>质检人:{{detail.qaer}}</p>
                    </div>
                    <div v-else>
                        <p>货件类型：{{ bringupType }}</p>
                        <p class="remark">备注:{{detail.remark}}</p>
                    </div>
                    <div>
                        <barcode :value="detail.inCode" tag="img" :width="1.5" :height="40" :fontSize="14" v-if="title === '采购入库单' " /> 
                        <barcode :value="detail.purchaseOrder" tag="img" :width="1.5" :height="40" :fontSize="14" v-else-if="title === '质检异常单' " />  
                        <barcode :value="detail.ticketCode" tag="img" :width="1.5" :height="40" :fontSize="14" v-else />           
                    </div>
                    <div  :class="{'new-width': title === '质检异常单' }">
                        <p v-if="title !== '采购入库单' ">制单人:{{ detail.createUser }}</p>
                        <p>{{ date }}</p>
                    </div>
                </div>
            </Header>
