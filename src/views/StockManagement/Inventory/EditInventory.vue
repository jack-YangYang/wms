<template>
    <main>
        <detail-title title="修改盘点" />
        <detail-header>
            <span>盘点单详情</span>
            <span v-html="msg" class="warning"></span>
        </detail-header>
        <left-content :showDateAndCode="true" :detail="detail" />
        <search-sku @doFilterSku="doFilterSku" :isDisabled="!show" />
        <detail-table :tableData="detailItems" :disabled="!show" :state="detail.state" />
...
export default {
    ...
    methods: {
        ...
        ...mapActions('Inventory', ['getContent', 'addIninventory', 'findSkuDetail']),
        ...
        saveData() {
            ...
            if (!this.detailItems.every(item => item.skuPrice)) {
                message('warning', '调拨数量不允许为空并且只能为整数数')
                return
            } 
            let data = {
            ...
            this.addIninventory({ data, ticketCode: this.ticketCode })
        },
        /* 导出数据 */
        exportData() {
            import("@/vendor/Export2Excel").then(excel => {
                // 设置导出表格的头部
                const tHeader = ['库存SKU', '盘点数量', '盘点单价', '盘点总金额']
                // 设置要导出的属性
                const filterVal = ['sku', 'skuCount', 'skuPrice', 'skuAmount']
                // 获取当前展示的表格数据
